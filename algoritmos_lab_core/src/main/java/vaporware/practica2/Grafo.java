...
public class Grafo<X, Y> {
    ...
    public void nuevoVertice(X v) {
        int hueco = hayHueco();
        if (hueco > -1 && posVertice(v) == -1) {
            vertices[hueco] = v;
        } else {
            System.out.print("esta lleno o ya esta");
        }
    }
    ...
    public ArrayList<X> adyacentes(X v) {
        ...
        if (pos > -1) {
            for (int col = 0; col < vertices.length; col++) {
                if (arcos[pos][col] != null) {
                    vert.add(vertices[col]);
                }
            }
        }
        return vert;
    ...
    public ArrayList<X> incidentes(X v) {
        ...
        if (pos > -1) {
            for (int fil = 0; fil < vertices.length; fil++) {
                if (arcos[fil][pos] != null) {
                    vert.add(vertices[fil]);
                }
            }
        }
        return vert;
    ...
    public Y peso(X origen, X destino) {
        ...
        if (posOr == -1 || posDest == -1) {
            ...
            if (arcos[posOr][posDest] == null) {
                System.out.println("No existe el arco " + origen + "-" + destino + ". Devuelvo null");
            } else {
                peso = arcos[posOr][posDest];
            }
        }
        return peso;
    }
    ...
    public boolean perteneceArco(X origen, X destino) {
        ...
        int posOr = posVertice(origen);
        int posDest = posVertice(destino);
        if (posOr == -1 || posDest == -1 || arcos[posOr][posDest] == null) {
            esta = false;
        }
        return esta;
    }
    ...
    private int posVertice(X v) {
        ...
        for (int n = 0; n < vertices.length && pos == -1; n++) {
            if (vertices[n] != null && vertices[n].equals(v)) {
                pos = n;
            }
        }
        return pos;
    ...
    public String toString() {
        ...
        for (int v = 0; v < vertices.length; v++) {
            if (vertices[v] != null) {
                s = s + String.format("%10s", vertices[v]);
            }
        }
        for (int f = 0; f < vertices.length; f++) {
            if (vertices[f] != null) {
                ...
                for (int c = 0; c < vertices.length; c++) {
                    if (vertices[c] != null) {
                        if (arcos[f][c] == null) {
                            ...
                            s = s + String.format("%10s", arcos[f][c]);
                        }
                    }
                }
            }
